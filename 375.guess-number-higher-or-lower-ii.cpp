...
public:
   /**
    ...
    int getMoneyAmount(int n) {
        // dp[i][j]: min cost to win [i:j]
        // = min{(k + 1) + max(dp[i][k - 1], dp[k + 1][j]) | i <= k <= j}
        ...
        for (int len = 1; len <= n; ++len) {
            for (int i = 0; i <= n - len; ++i) {
                ...
                for (int k = i; k <= j; ++k) {
                    if (k == j)
                        cost = min(cost, k + 1 + dp[i][k - 1]);
                    else if (k == i)
                        cost = min(cost, k + 1 + dp[k + 1][j]);
                    else
                        cost = min(cost, k + 1 + max(dp[i][k - 1], dp[k + 1][j]));
                }
                dp[i][j] = cost;
            }
        }
