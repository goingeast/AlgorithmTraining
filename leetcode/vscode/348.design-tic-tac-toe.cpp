...
private:
    ...
    vector<vector<int>> diagcnt;
    int n;
public:
    /** Initialize your data structure here. */
    TicTacToe(int n): rowcnt(n, vector<int>(2, 0)), colcnt(n, vector<int>(2, 0)), diagcnt(2, vector<int>(2, 0)), n(n) {
    }
    ...
    int move(int row, int col, int player) {
        if(rowcnt[row][(player+2)%2] && colcnt[row][(player+2)%2]) {
            return 0;
        }
        player--;
        ++rowcnt[row][player];
        ...
        if (rowcnt[row][player] == n || colcnt[col][player] == n || diagcnt[0][player] == n || diagcnt[1][player] == n){
            return ++player;
        }
        return 0;
    }
